...
RESTART_COUNT = 0 
MAX_RESTART_COUNT = 15 # 获取了属于11.11.11.0/24网段的IP 
...
if ["$i" -eq "$last"]; then 
echo " jump local ip: $LOCAL_IP" else 
...
((RESTART_COUNT++)) 
fi done if [$RESTART_COUNT -eq $MAX_RESTART_COUNT]; then echo "Restarting the system..." reboot fi sleep 60 done